...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        int host_result = pot(source_b[i],4)*source_a[i] + 4*pot(source_f[i],3)*source_b[i]*source_g[i] + 6*pot(source_b[i],2)*pot(source_f[i],2)*source_c[i] + pot(source_f[i],4)*source_e[i] + 4*pot(source_b[i],3)*source_f[i]*source_d[i];
