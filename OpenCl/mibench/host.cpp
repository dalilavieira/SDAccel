...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        int host_result = source_c[i] + 9*pot(source_c[i],2) + pot(source_a[i],2) + 43*source_a[i] + 6*source_a[i]*source_c[i] + 43*source_b[i] + 6*source_b[i]*source_c[i] + pot(source_b[i],2) + 2*source_a[i]*source_b[i];
