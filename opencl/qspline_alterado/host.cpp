...
int main(int argc, char **argv) {
    ...
    vector<int, aligned_allocator<int>> source_d(DATA_SIZE, 1);
    vector<int, aligned_allocator<int>> source_e(DATA_SIZE, 1);
    vector<int, aligned_allocator<int>> source_f(DATA_SIZE, 1);
    vector<int, aligned_allocator<int>> source_g(DATA_SIZE, 1);
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(narg++, buffer_d));
    OCL_CHECK(err, err = krnl_vector_add.setArg(narg++, buffer_e));
    OCL_CHECK(err, err = krnl_vector_add.setArg(narg++, buffer_f));
    OCL_CHECK(err, err = krnl_vector_add.setArg(narg++, buffer_g));
    ...
    // DDR memory.
    OCL_CHECK(err,
              err = q.enqueueMigrateMemObjects({buffer_a, buffer_b, buffer_c, buffer_d, buffer_e, buffer_f, buffer_g},
                                               0 /* 0 means from host*/));
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        int host_result = (source_b[i]*source_b[i]*source_b[i]*source_b[i]*source_a[i]) + 4*(source_f[i]*source_f[i]*source_f[i]*source_b[i]*source_g[i]) + 6*(source_b[i]*source_b[i])*(source_f[i]*source_f[i])*source_c[i]
        + (source_f[i]*source_f[i]*source_f[i]*source_f[i]*source_e[i]) + 4*(source_b[i]*source_b[i]*source_b[i]*source_f[i]*source_d[i]);
