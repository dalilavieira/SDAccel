...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        int host_result = source_c[i] + 9*(source_c[i]*source_c[i]) + (source_a[i]*source_a[i]) + 43*source_a[i] + 6*source_a[i]*source_c[i] + 43*source_b[i] + 6*source_b[i]*source_c[i] + (source_b[i]*source_b[i]) + 2*source_a[i]*source_b[i];
