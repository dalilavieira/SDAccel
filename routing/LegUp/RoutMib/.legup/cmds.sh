...
echo "create_clock -period 5 -name clk [get_ports clk]" > ./synthesis/RoutMib.sdc
# echo "create_clock -period 1 -name clk2x [get_ports clk2x]" >> ./synthesis/RoutMib.sdc
# echo "create_clock -period 2 -name OSC_50_BANK2 [get_ports OSC_50_BANK2]" >> ./synthesis/RoutMib.sdc
# these contraints only apply to Intel
echo "derive_pll_clocks" >> ./synthesis/RoutMib.sdc
echo "derive_clock_uncertainty" >> ./synthesis/RoutMib.sdc
cd ./synthesis/; quartus_sh \
  -t /usr/lib/legup-7.5/legup/examples/setup_proj.tcl \
  "Arria10" Arria10-PAC RoutMib top 0 ; \
  cd ..
