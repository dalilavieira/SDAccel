...
echo "create_clock -period 5 -name clk [get_ports clk]" > ./synthesis/RoutP5.sdc
# echo "create_clock -period 1 -name clk2x [get_ports clk2x]" >> ./synthesis/RoutP5.sdc
# echo "create_clock -period 2 -name OSC_50_BANK2 [get_ports OSC_50_BANK2]" >> ./synthesis/RoutP5.sdc
# these contraints only apply to Intel
echo "derive_pll_clocks" >> ./synthesis/RoutP5.sdc
echo "derive_clock_uncertainty" >> ./synthesis/RoutP5.sdc
cd ./synthesis/; quartus_sh \
  -t /usr/lib/legup-7.5/legup/examples/setup_proj.tcl \
  "Arria10" Arria10-PAC RoutP5 top 0 ; \
  cd ..
