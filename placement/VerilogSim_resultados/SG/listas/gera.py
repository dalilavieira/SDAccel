...
def gera_exData(x,lista,pref):
	arq_nome = pref+"e"+x+"Data.txt"
	#print(arq_nome)
...
if len(sys.argv) > 1:
    ...
    for i in range(101):
        try:
            file = sys.argv[1]+"/"+sys.argv[1]+"_"+str(i)+".in"
            print(file)

            vsdestino = []
            vsorigem = []
            n_v = le_arquivo(vsorigem,vsdestino,file)
            p = gera_exData("a",vsorigem,sys.argv[1]+"out/"+sys.argv[1]+"_"+str(i))
            p = gera_exData("b",vsdestino,sys.argv[1]+"out/"+sys.argv[1]+"_"+str(i))
            #gera_gridData(p)
            #print("n v ",n_v)
            #gera_posData(p,n_v)
        except Exception as e:
